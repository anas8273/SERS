...
export default function DynamicEditorPage() {
  ...
  const handleAIChat = async () => {
    ...
    try {
      ...
      setAIMessages(prev => [...prev, { role: 'assistant', content: 'حدث خطأ في الاتصال بالذكاء الاصطناعي.' }]);
    } finally {
      setAILoading(false);
    }
  };
            ...
            <label htmlFor={`file-${field.id}`} className="cursor-pointer">
              {value ? (
                ...
                <div className="text-gray-400">
                  ...
                  <span className="text-sm">{field.type === 'image' ? 'ارفع صورة' : 'ارفع ملف'}</span>
                </div>
              )}
            </label>
          </div>
      ...
      <div className="flex flex-col-reverse lg:flex-row h-[calc(100vh-64px)]">
        ...
        <div className="w-full lg:w-[400px] bg-white border-t lg:border-t-0 lg:border-l border-gray-200 overflow-y-auto flex-shrink-0 max-h-[50vh] lg:max-h-none">
          ...
          <div className="flex border-b border-gray-200 sticky top-0 bg-white z-10">
            {[
              { id: 'fields' as const, label: 'الحقول', icon: '📝' },
              { id: 'design' as const, label: 'التصميم', icon: '🎨' },
            ...
            {activeTab === 'ai' && (
              <div className="space-y-4">
                <div className="bg-purple-50 rounded-xl p-4">
                  <h3 className="text-sm font-bold text-purple-800 mb-1">المساعد الذكي</h3>
                  <p className="text-xs text-purple-600">اسأل المساعد الذكي عن أي شيء يتعلق بهذا القالب</p>
                </div>
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                  {aiMessages.length === 0 && (
                    <div className="text-center py-8 text-gray-400">
                      ...
                      <p className="text-sm">ابدأ محادثة مع المساعد الذكي</p>
                    </div>
                  )}
