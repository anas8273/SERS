 ...
 * 
 * MOBILE FIXES:
 * - Mobile menu uses fixed full-screen overlay (no clipping)
 * - User dropdown uses fixed positioning on mobile
 * - Notification bell delegates to its own fixed dropdown
 * - All dropdowns are body-level portals, not nested inside header overflow
 */
export function Navbar() {
    ...
    return (
        <>
            ...
            {/* ═══ USER DROPDOWN — Rendered as a portal at body level ═══ */}
            {/* This prevents clipping from any parent overflow/transform issues */}
            {userMenuOpen && (
                <>
                    <div
                    ...
                    />
                    {/* MOBILE: centered card, DESKTOP: anchored top-right area */}
                    <div 
                        className={cn(
                            "fixed z-[70] w-60 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl shadow-gray-200/50 dark:shadow-gray-900/50 border border-gray-100 dark:border-gray-700 py-2 overflow-hidden",
                            // Mobile: center horizontally with margin
                            "inset-x-3 top-[4.5rem] sm:inset-x-auto",
                            // Desktop: position at top right
                            "sm:top-[4.5rem]",
                            dir === 'rtl' ? 'sm:right-4 lg:right-auto lg:left-4' : 'sm:right-4'
                        )}
            ...
            {/* ═══ MOBILE MENU — Full-screen overlay (no clipping possible) ═══ */}
            {/* Rendered outside <header> to avoid any overflow/transform containment */}
            {mobileMenuOpen && (
                <>
                    {/* Dark backdrop */}
                    <div
                        ...
                        aria-hidden="true"
                    />
                    {/* Menu panel — slides down from navbar, COMPLETELY outside header */}
                    <div
                        id="mobile-menu"
                        role="dialog"
                        aria-modal={true}
                        aria-label="القائمة الرئيسية"
                        className="lg:hidden fixed left-0 right-0 z-[56] bg-white dark:bg-gray-900 backdrop-blur-2xl border-b border-gray-200/60 dark:border-gray-700/60 shadow-2xl shadow-gray-900/20"
                        style={{
                            top: '4rem',
                            ...
                            {isAuthenticated ? (
                                <div className="flex items-center justify-between px-1">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-primary/10 text-primary rounded-full flex items-center justify-center font-bold text-sm border-2 border-primary/20">
                                            {user?.name?.charAt(0) || <User className="w-4 h-4" />}
                                        </div>
                                        <div>
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 text-sm text-red-500 font-semibold px-3 py-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                    >
                                        <LogOut className="w-4 h-4" />
                                        {t('nav.logout')}
                                    </button>
