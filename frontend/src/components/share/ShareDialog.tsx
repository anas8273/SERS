...
export function ShareDialog({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={ta('مشاركة', 'Share')} size="md">
      <div className="space-y-6">
        ...
        {/* Copy Link */}
        <div>
          <label className="block text-sm font-medium mb-2">{ta('رابط المشاركة', 'Share Link')}</label>
          <div className="flex gap-2">
            ...
            >
              {copied ? (
                <>
                  <Check className="w-4 h-4 ms-2 text-green-500" />
                  {ta('تم النسخ', 'Copied')}
                </>
              ) : (
                <>
        ...
        <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm">{ta('مشاركة عبر رمز QR', 'Share via QR Code')}</span>
          </div>
