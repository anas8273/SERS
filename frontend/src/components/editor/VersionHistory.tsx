...
export function VersionHistory({ 
  ...
  const fetchVersionHistory = async () => {
    ...
    try {
      ...
      toast.error(ta('فشل في تحميل تاريخ الإصدارات', 'Failed to load version history'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRestore = async (version: Version) => {
    setIsRestoring(true);
    try {
      // [QUALITY-02 FIX] Use canonical restoreRecordVersion() — restoreVersion() was removed as a duplicate
      const response = await api.restoreRecordVersion(recordId, version.id);
      if (response.success) {
      ...
      toast.error(ta('فشل في استرداد الإصدار', 'Failed to restore version'));
    } finally {
  ...
  const handleExportVersion = async (version: Version) => {
    try {
      ...
      if (response.success && response.data?.url) {
        ...
        toast.success(ta('تم تصدير الإصدار بنجاح', 'Version exported successfully'));
      }
    } catch (error: any) {
      logger.error('Export Version Error:', error);
      toast.error(ta('فشل في تصدير الإصدار', 'Failed to export version'));
    }
  };
        ...
        <div className="space-y-2">
          {fields.map(([key, value]) => (
            <div key={key} className="text-sm">
              ...
              <span className="text-gray-900 dark:text-white me-2">
                {typeof value === 'string' 
                  ? value.length > 50 
        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh]" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-xl">
              ...
              {ta('تاريخ الإصدارات', 'Version History')}
            </DialogTitle>
            <DialogDescription>
            ...
            {isLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="text-center space-y-3">
                  ...
                  <p className="text-sm text-gray-500">{ta('جاري تحميل تاريخ الإصدارات...', 'Loading version history...')}</p>
                </div>
              </div>
            ) : versions.length === 0 ? (
              <div className="text-center py-12 space-y-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white mb-2">
                    {ta('لا توجد إصدارات سابقة', 'No previous versions')}
                  </h3>
                  <p className="text-sm text-gray-500">
                    {ta('سيتم حفظ الإصدارات تلقائياً عند إجراء تغييرات على القالب', 'Versions will be saved automatically when you make changes')}
                  </p>
                </div>
              </div>
            ) : (
        ...
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              {ta('تأكيد الاسترداد', 'Confirm Restore')}
            </AlertDialogTitle>
            <AlertDialogDescription>
              هل أنت متأكد من استرداد الإصدار {selectedVersion?.version_number}؟
              <br />
              {ta('سيتم استبدال البيانات الحالية بالبيانات من هذا الإصدار.', 'Current data will be replaced with this version.')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{ta('إلغاء', 'Cancel')}</AlertDialogCancel>
            <AlertDialogAction
        ...
        <DialogContent className="sm:max-w-lg" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {ta('معاينة الإصدار', 'Preview Version')}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-gray-100 dark:border-gray-800">
                {!selectedVersion.is_current && (
                  ...
                  >
                    <RotateCcw className="w-4 h-4" />
                    {ta('استرداد هذا الإصدار', 'Restore This Version')}
                  </Button>
                )}
