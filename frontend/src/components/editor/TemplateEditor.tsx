...
export function TemplateEditor({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        logger.error('Error listening to record:', error);
        toast.error(ta('حدث خطأ في تحميل السجل', 'Error loading record'));
      }
    );
  ...
  const saveToFirestore = useCallback(
    debounce(async (newData: Record<string, any>) => {
      ...
      try {
        ...
        toast.error(ta('فشل حفظ التغييرات', 'Failed to save changes'));
      }
    }, 1500),
    [recordId, title]
  );
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      toast.error(ta('يرجى إدخال عنوان للقالب', 'Please enter a template title'));
      return;
    }
    ...
    try {
      ...
      setLastSaved(new Date());
      setHasChanges(false);
      toast.success(ta('تم حفظ التغييرات بنجاح', 'Changes saved successfully'));
    } catch (error: any) {
  ...
  const handleExport = async (format: 'image' | 'pdf') => {
    if (!recordId) {
      toast.error(ta('يرجى حفظ القالب أولاً', 'Please save the template first'));
      return;
    }

    setExporting(true);
    try {
      const response = await api.post(`/templates/export/${recordId}`, { format }) as any;
      ...
      toast.error(ta('حدث خطأ أثناء التصدير', 'Error during export'));
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleAIFillAll = async () => {
    ...
    try {
      ...
      logger.error('AI Fill All Error:', error);
      toast.error(ta('حدث خطأ في الذكاء الاصطناعي', 'AI error occurred'));
    } finally {
      setAiLoading(false);
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => handleExport('image')}>
                    ...
                    {ta('تصدير كصورة PNG', 'Export as PNG')}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleExport('pdf')}>
                    <FileText className="w-4 h-4 me-2" />
                    {ta('تصدير كـ PDF', 'Export as PDF')}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
          ...
          <div className="lg:col-span-2">
            <Card className="shadow-sm">
              <CardHeader className="border-b">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">{ta('بيانات القالب', 'Template Data')}</CardTitle>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="sm">
                          ...
                          {ta('تغيير التصميم', 'Change Design')}
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-gray-400">
                    <div className="text-center">
                      ...
                      <p className="text-sm">{ta('معاينة القالب', 'Preview Template')}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {ta('ملء ذكي بالذكاء الاصطناعي', 'AI Smart Fill')}
            </DialogTitle>
            <DialogDescription>
              {ta('سيقوم الذكاء الاصطناعي بملء جميع الحقول تلقائياً بناءً على عنوان القالب والسياق.', 'AI will automatically fill all fields based on template title and context.')}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
              <h4 className="font-bold text-sm mb-2 flex items-center gap-2">
                ...
                {ta('ما سيتم ملؤه:', 'What will be filled:')}
              </h4>
