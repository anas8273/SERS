...
export function SmartFieldInput({
  ...
  const getSuggestion = async () => {
    ...
    try {
      ...
      toast.error(ta('حدث خطأ في الاتصال بالذكاء الاصطناعي', 'AI connection error occurred'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const acceptSuggestion = () => {
    if (suggestion) {
      ...
      toast.success(ta('تم تطبيق الاقتراح بنجاح', 'Suggestion applied successfully'));
    }
  };
  ...
  const copySuggestion = () => {
    if (suggestion) {
      ...
      toast.success(ta('تم نسخ الاقتراح', 'Suggestion copied'));
    }
  };
  ...
  const appendSuggestion = () => {
    if (suggestion) {
      ...
      setSuggestion(null);
      setShowSuggestion(false);
      toast.success(ta('تم إضافة الاقتراح', 'Suggestion added'));
    }
  };
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="font-bold text-sm text-gray-900 dark:text-white">
                          {ta('اقتراح ذكي', 'Smart Suggestion')}
                        </h4>
                        <p className="text-xs text-gray-500">{ta('مدعوم بـ GPT-4', 'Powered by GPT-4')}</p>
                      </div>
                    </div>
                    <Button
                      variant="ghost"
