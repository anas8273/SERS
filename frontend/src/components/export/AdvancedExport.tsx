  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="lg">
      <div className="space-y-6">
        {/* Format Selection */}
        <div>
          <label className="block text-sm font-medium mb-3">{ta('صيغة التصدير', 'Export Format')}</label>
        ...
        {(options.format === 'png' || options.format === 'jpg') && (
          <div>
            <label className="block text-sm font-medium mb-3">{ta('جودة الصورة', 'Image Quality')}</label>
        ...
        {options.format === 'pdf' && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-3">{ta('حجم الورق', 'Paper Size')}</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium mb-3">{ta('الاتجاه', 'Direction/Orientation')}</label>
              <select
                ...
                className="w-full p-2 rounded-lg border bg-background"
              >
                <option value="portrait">{ta('عمودي', 'Portrait')}</option>
                <option value="landscape">{ta('أفقي', 'Landscape')}</option>
              </select>
            </div>
          </div>
        )}
        ...
        >
          ...
          {ta('خيارات متقدمة', 'Advanced Options')}
        </button>
        ...
        {showAdvanced && (
          <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center justify-between">
              <label className="text-sm">{ta('تضمين الترويسة', 'Include Header')}</label>
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between">
              <label className="text-sm">{ta('تضمين التذييل', 'Include Footer')}</label>
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between">
              <label className="text-sm">{ta('إضافة علامة مائية', 'Add Watermark')}</label>
              <input
                type="checkbox"
            ...
            {options.includeWatermark && (
              <div>
                <label className="block text-sm mb-2">{ta('نص العلامة المائية', 'Watermark Text')}</label>
                <input
                  type="text"
