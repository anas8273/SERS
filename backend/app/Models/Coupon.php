...
class Coupon extends Model
{
    ...
    /**
     * Record usage of this coupon atomically.
     * Uses DB::lock to prevent race conditions under concurrent requests.
     */
    public function recordUsage(string $userId, string $orderId, float $discountAmount): CouponUsage
    {
        // Re-validate with a pessimistic lock to prevent race conditions
        // This is called inside PurchaseService::createOrder() which is already in a DB::transaction()
        ...
        // Re-check usage limit with locked data
        if ($locked->max_uses !== null && $locked->used_count >= $locked->max_uses) {
            throw new \Exception('كود الخصم تجاوز الحد الأقصى للاستخدام');
        }
