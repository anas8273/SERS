...
class ExportController extends Controller
{
    ...
    {
        // Intervention\Image is optional — image export features degrade gracefully
    ...
    public function toPdf(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Authorization: free templates are accessible to all authenticated users;
            // paid templates require ownership (purchased via order → user_libraries)
            if (!$template->is_free) {
                $owns = \App\Models\UserLibrary::userOwnsTemplate($user->id, $template->id);
                if (!$owns) {
                    return response()->json([
                        'success' => false,
                        'message' => 'يجب شراء القالب أولاً للتمكن من تصديره',
                        'error' => 'not_purchased',
                    ], 403);
                }
            }
            ...
            return response()->json([
                'success' => false,
                'message' => 'حدث خطأ أثناء تصدير القالب. يرجى المحاولة مرة أخرى',
                'error' => 'export_failed',
            ], 500);
        }
    }
    ...
    {
        $templateTitle = $title ?? $template->name_ar ?? 'قالب تعليمي';
