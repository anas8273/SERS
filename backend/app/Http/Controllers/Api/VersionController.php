...
class VersionController extends Controller
{
     ...
     */
    public function getVersionHistory(Request $request, string $recordId): JsonResponse
    {
        // [FIX-BUG03] Validate UUID format before passing to the service.
        // Without this, a plain integer like "123" would reach Firestore and generate
        // an obscure internal error instead of a clear 422 response.
        if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i', $recordId)) {
            return response()->json([
                'success' => false,
                'message' => 'معرّف السجل غير صالح — يجب أن يكون UUID',
                'error'   => 'invalid_record_id',
            ], 422);
        }

        // Verify ownership — only return versions for records belonging to the current user
        $record = \App\Models\UserTemplateData::where('id', $recordId)
    ...
    {
        // [FIX-BUG03] UUID validation — prevents obscure service errors
        if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i', $recordId)) {
            return response()->json([
                'success' => false,
                'message' => 'معرّف السجل غير صالح — يجب أن يكون UUID',
        ...
        if (!$record) {
            return response()->json([
                'success' => false,
                'message' => 'السجل غير موجود أو غير مصرح',
                'error'   => 'not_found',
            ], 404);
        }
