...
class InteractivePDFAutomationService
{
    ...
    private function createOrUpdateTemplate(array $schema): Template
    {
        ...
        return Template::updateOrCreate(
            ...
            [
                ...
                'slug' => $this->generateSlug($schema['name_ar'] ?? $schema['template_id']),
    ...
    {
        $category = \App\Models\Category::where('name_ar', $this->translateServiceType($serviceType))->first();
    ...
    {
        $section = \App\Models\Section::where('name_ar', 'النماذج التلقائية')->first();
    ...
    {
        $templateName = str_replace(' ', '_', $template->name_ar ?? 'template');
